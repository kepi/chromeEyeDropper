const NEED_BG_VERSION = 16; // minimum version of bg script we need
...
function initPickButton(tab) {
  ...
  // chrome gallery
    ...
    message =
      "Chrome doesn't allow its <i>extensions</i> to play on Web Store.";
    pickEnabled = false;
  }
  // local pages
  else if (tab.url.indexOf("file") == 0) {
    message =
      "Chrome doesn't allow its <i>extensions</i> to play with your local pages.";
    pickEnabled = false;
  }
...
function drawColorHistory() {
  ...
  let history_tool_noempty_els = document.getElementsByClassName(
  ...
  if (palette.colors.length > 0) {
    ...
    instructions_el.innerHTML =
      "History is empty, try to pick some colors first.";
    for (n of history_tool_noempty_els) {
...
function exportHistory() {
  ...
  if (bgPage.bg.plus()) {
    ...
    for (let color of history) {
      let d =
        typeof color.t === "function" ? new Date(color.t()) : new Date(color.t);
      let datestring = `${d.getFullYear()}-${("0" + (d.getMonth() + 1)).slice(
        -2
      )}-${("0" + d.getDate()).slice(-2)} ${("0" + d.getHours()).slice(-2)}:${(
        "0" + d.getMinutes()
      ).slice(-2)}:${("0" + d.getSeconds()).slice(-2)}`;

      csv += `"${color.h}","${datestring}","${
        bgPage.bg.color_sources[color.s]
      }"`;
  ...
  if (!content_found) {
    ...
    request.onload = () => {
      if (request.status >= 200 && request.status < 400) {
        ...
        console.error(
          `Error loading ${tab.id} content through AJAX: ${request.status}`
        );
      }
    };
