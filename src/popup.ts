  ...
  // local pages
    ...
    chrome.extension.isAllowedFileSchemeAccess((isAllowedAccess) => {
      if (isAllowedAccess) {
        ...
        initExternalLink(
          document.getElementById("link-help-file-urls") as HTMLLinkElement
        )
      }
    })
  } else {
...
function drawColorPalettes() {
  ...
  const paletes = document.getElementsByClassName(
    "ed-palette"
  ) as HTMLCollectionOf<HTMLElement>
  ...
  if (!content_found) {
    ...
    request.onload = () => {
      if (request.status >= 200 && request.status < 400) {
        ...
        console.error(
          `Error loading ${tabId} content through AJAX: ${request.status}`
        )
      }
    }
...
function setColor(color: string, history = true, source = 1, url?: string) {
  ...
  if (bgPage.bg.settings.autoClipboard) {
    const color_with_o_wo_grid = bgPage.bg.settings.autoClipboardNoGrid
      ? color.substring(1)
      : color
