<script lang="ts">
  ...
  let active = $derived(($pStore.active?.id ?? 0) === palette.id && $popupDialog != "palettes")
</script>
...
>
  ...
  >
    <div class="truncate max-w-[48px] shadow-black" class:text-shadow={active}>{palette.name}</div>
  </button>
</div>
