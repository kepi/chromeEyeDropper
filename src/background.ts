...
var bg = {
  ...
  saveToHistory: function (color: string, source = 1, url?: string) {
    ...
    if (
      ...
      palette.colors.push(
        bg.historyColorItem(color, Date.now(), source, false, url)
      )
      console.info(
        "Color " + color + " saved to palette " + bg.getPaletteName()
      )
      bg.saveHistory()
    } else {
      console.info(
  ...
  changePalette: function (palette_name: string) {
    if (bg.history.current_palette === palette_name) {
      ...
      console.error(
        "Cannot switch to palette " + palette_name + ". Palette not found."
      )
    }
  },
  ...
  uniquePaletteName: function (name: string) {
    ...
    if (name === undefined || !name || name.length < 1) {
      ...
      if (matches[2] === undefined) {
        console.info(
          "uniquePaletteName: " + name + " occupied, trying '" + name + "1'"
        )
        ...
        console.info(
          "uniquePaletteName: " + name + " occupied, trying '" + new_name + "'"
        )
  ...
  createPalette: function (name: string) {
    ...
    console.info(
      "Creating new palette " + name + ". Unique name: " + palette_name
    )
  ...
  tryConvertOldSettings: function () {
    ...
    bg.settings.dropperCursor =
      window.localStorage.dropperCursor === "crosshair"
        ? "crosshair"
        : "default"
    // sync settings
    bg.saveSettings()
    ...
    for (
      var _i = 0, setting_keys_1 = setting_keys;
      _i < setting_keys_1.length;
      _i++
    ) {
