...
var bg = {
    ...
    saveToHistory: function (color, source, url) {
        ...
        if (!palette.colors.find(function (x) { return x.hex == color; })) {
    ...
    isPalette: function (name) {
        return bg.history.palettes.find(function (x) { return x.name == name; }) ? true : false;
    },
    getPalette: function (name) {
        ...
        return bg.history.palettes.find(function (x) { return x.name == name; });
    },
    ...
    getPaletteNames: function () {
        return bg.history.palettes.map(function (x) { return x.name; });
    },
    uniquePaletteName: function (name) {
        ...
        else if (bg.getPaletteNames().find(function (x) { return x == name; })) {
     ...
     * f = favorite
     */
    historyColorItem: function (color, timestamp, source, source_url, favorite) {
        if (timestamp === void 0) { timestamp = Date.now(); }
        if (source === void 0) { source = 1; }
        if (source_url === void 0) { source_url = null; }
    ...
    tryConvertOldSettings: function () {
        ...
        var setting_keys = ['autoClipboard', 'autoClipboardNoGrid', 'enableColorTooltip', 'enableColorToolbox', 'enableRightClickDeactivate', 'dropperCursor'];
